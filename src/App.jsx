...
function AuthModal({ onClose, onLogin }) {
  ...
  const submit = () => {
    ...
    onLogin({ name: name || email.split("@")[0], email, role: role || "buddy", avatar: "https://i.pravatar.cc/80?img=12", isNew: mode === "signup" });
  };
  ...
  return (
    <div className="modal-bg" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <button className="btn-main" onClick={submit} style={{ opacity: mode === "signup" && !role ? 0.5 : 1 }}>
...
export default function App() {
  ...
  const profileDone = form => { if (form.avatar) setUser(u => ({ ...u, avatar: form.avatar })); setScreen("under-review"); };
  const send = (id, text) => setMessages(p => ({ ...p, [id]: [...(p[id] || []), { from: "me", text }] }));
  const openChat = p => { setViewProfile(null); setChatProfile(p); if (!likes.includes(p.id)) setLikes(l => [...l, p.id]); setTab("messages"); };
